...
const assert = (condition, message) => {
  if (!condition) throw new Error(`Vercel packaging check failed: ${message}`);
};
...
assert(rootFunction.includeFiles === wildcardFunction.includeFiles, 'API adapters must use the same includeFiles configuration');
for (const requiredPath of ['nest-api/dist/**', 'nest-api/prisma/**', 'node_modules/.prisma/client/**', 'node_modules/@prisma/client/**']) {
