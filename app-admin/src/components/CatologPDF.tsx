...
export const CatalogPDF: React.FC<CatalogPDFProps> = ({ produtos, storeName }) => (
  <Document>
        ...
        {produtos.map((p) => (
          <View key={p.id} style={styles.card} wrap={false}>
            <View style={styles.imageContainer}>
              {p.imageUrl ? (
                <Image src={{ uri: p.imageUrl, method: 'GET', headers: {}, body: '' }} style={styles.image} />
              ) : (
            ...
            <Text style={styles.description}>{p.description || "Sem descrição."}</Text>
