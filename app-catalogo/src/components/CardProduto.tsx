      ...
      <div className="relative aspect-[1/1.1] bg-gray-100 overflow-hidden">
        ...
        {temDesconto && (
          <div className="absolute top-2 left-2 bg-red-600 text-white text-[10px] font-black px-2 py-1 rounded-md shadow-sm flex items-center gap-1 animate-pulse">
            <TrendingDown size={12} />-{percentualDesconto}%
          </div>
        )}
        ...
        {/* BADGE: ESTOQUE */}
        {!temStock ? (
          <span className="absolute top-2 right-2 bg-gray-900 text-white text-[9px] font-bold px-2 py-1 rounded-md uppercase tracking-wider">
            Esgotado
          </span>
        ) : stock <= lowStockLimit ? (
          <span className="absolute top-2 right-2 bg-red-600/90 backdrop-blur-sm text-white text-[9px] font-bold px-2 py-1 rounded-md shadow-sm border border-white/20">
            Últimas {stock}
          </span>
        ) : null}
      </div>
      ...
      <div className="p-3.5 flex flex-col flex-grow justify-between bg-white">
        ...
        <div className="mt-3 flex items-end justify-between gap-2 border-t border-gray-50 pt-3">
          <div className="flex flex-col items-start">
            ...
            <p
              className={`text-lg font-black tracking-tight leading-none ${temDesconto ? 'text-red-600' : ''}`}
              style={!temDesconto ? { color: config.secondaryColor } : {}}
            >
          ...
          <button
            ...
            style={
              temStock
                ? {
                    backgroundColor: temDesconto
                      ? '#DC2626'
                      : config.primaryColor,
                  }
                : {}
            } // Botão vermelho se for promo
