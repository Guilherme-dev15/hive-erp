...
export function ImportModal({ isOpen, onClose, onSuccess }: ImportModalProps) {
  ...
  const handleImageUpload = async (id: string, file: File) => {
    ...
    setPreviewData((prev) =>
      prev.map((p) => (p.tempId === id ? { ...p, isUploading: true } : p))
    );
    ...
    try {
      ...
      setPreviewData((prev) =>
        prev.map((p) =>
          p.tempId === id ? { ...p, imageUrl: url, isUploading: false } : p
        )
      );
  ...
  const handleFinalSave = async () => {
    ...
    try {
      ...
      const payload = previewData.map(({ tempId, isUploading, ...rest }) => ({
        ...rest,
        category:
          categories.find((c) => c.id === rest.categoryId)?.name || 'Geral',
          ...
          >
            ...
            <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center flex-shrink-0">
              <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                ...
                {step === 'upload'
                  ? 'Importar Excel'
                  : `Revisar ${previewData.length} Produtos`}
              </h3>
              <button
                onClick={onClose}
                ...
                <div className="space-y-6">
                  <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 text-xs font-bold bg-white text-blue-600 px-3 py-2 rounded-lg border border-blue-200"
                    >
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center flex-shrink-0">
              {step === 'review' ? (
                <>
                  ...
                  <div className="flex gap-3">
                    <span className="text-xs text-gray-500 self-center hidden sm:block">
                      Certifique-se de que todas as fotos foram carregadas.
                    </span>
                    <button
                ...
                <button
                  onClick={onClose}
                  className="ml-auto px-4 py-2 text-gray-500 font-bold hover:bg-gray-200 rounded-lg"
                >
                  Cancelar
                </button>
              )}
