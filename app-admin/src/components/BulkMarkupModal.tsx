...
export const BulkMarkupModal: React.FC<BulkMarkupModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    useEffect(() => {
        ...
        const fetchCategories = async () => {
            try {
                ...
                const categoriasQuery = query(collection(db, "categories"), where("userId", "==", user.uid));
                const querySnapshot = await getDocs(categoriasQuery);
                const loadedCategories: Category[] = [];
                querySnapshot.forEach((doc) => {
                    loadedCategories.push({ id: doc.id, name: doc.data().name as string });
                });
    ...
    const handleApplyMarkup = async () => {
        ...
        try {
            const updatedCount = await updateMarkupViaFirebase(Number(markupValue), selectedCategory);
