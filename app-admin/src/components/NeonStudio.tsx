  ...
  const addVariante = (itemId: string, type: 'cm' | 'aro') => {
    setItems((prev) =>
      prev.map((item) => {
        ...
        return {
          ...item,
          data: { ...item.data, variantes: [...item.data.variantes, nova] },
        };
      })
    );
  };
  ...
  const handleSync = async () => {
    ...
    for (let i = 0; i < toUpload.length; i++) {
      ...
      try {
        ...
        const fileToUpload = new File([finalBlob], `${finalName}.jpg`, {
          type: 'image/jpeg',
        });
        ...
        const productData = {
          ...
          category:
            categories.find((c) => c.id === item.data.categoryId)?.name ||
            'Geral',
          ...
          <header className="px-6 py-4 border-b border-white/5 bg-black/80 backdrop-blur-xl flex flex-col xl:flex-row justify-between items-center sticky top-0 z-50 gap-4 shadow-2xl">
            ...
            <div className="flex items-center bg-white/5 p-1.5 rounded-2xl border border-white/10 shadow-inner overflow-x-auto w-full xl:w-auto scrollbar-hide backdrop-blur-md">
              <div className="flex items-center gap-3 px-3 border-r border-white/5">
                <div className="flex flex-col w-36">
                  <label className="text-[9px] text-gray-500 font-bold uppercase mb-1">
                    Fornecedor
                  </label>
                  <select
              ...
              <div className="flex items-center gap-3 px-3 border-r border-white/5">
                <div className="flex flex-col w-28">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleGlobalChange('subcategory', e.target.value)
                    }
              ...
              <div className="flex items-center gap-3 px-3 border-r border-white/5">
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${showMetalCalc ? 'bg-purple-600 text-white' : 'bg-white/5 text-gray-500'}`}
                >
                  <Scale size={14} /> {showMetalCalc ? 'Modo Peso' : 'Modo R$'}
                </button>
                {showMetalCalc && (
                  <div className="flex flex-col w-24">
                    <label className="text-[9px] text-purple-400 font-bold uppercase mb-1">
                      R$/g
                    </label>
                    <input
                      type="number"
              ...
              <div className="flex items-center gap-4 px-3">
                <div className="flex flex-col w-20 text-center">
                  <label className="text-[9px] text-cyan-500 font-bold uppercase mb-1">
                    Markup
                  </label>
                  <input
                    type="number"
                    ...
                    className="bg-transparent text-sm font-bold text-white outline-none text-center"
                  />
                </div>
                <div className="flex flex-col w-16 text-center">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleGlobalChange('stock', e.target.value)
                    }
                    className="bg-transparent text-sm font-bold text-white outline-none text-center"
                  />
                </div>
            ...
            <div className="flex items-center gap-3 w-full xl:w-auto justify-end">
              ...
              <button
                onClick={handleSync}
                disabled={
                  isUploading || items.filter((i) => i.selected).length === 0
                }
                className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-cyan-600 to-blue-600 text-white rounded-xl font-bold shadow-[0_0_20px_rgba(6,182,212,0.3)] disabled:opacity-50 transition-all active:scale-95"
              >
                {isUploading ? (
...
function ProductCard({
  ...
  return (
    <div
      className={`relative bg-black rounded-2xl overflow-hidden flex flex-col transition-all duration-300 border border-white/10 group hover:border-white/30 ${isSelected ? 'ring-1 ring-cyan-500/50' : 'opacity-90 hover:opacity-100'}`}
    >
      {/* IMAGEM E EDITOR */}
      <div className="relative aspect-square bg-[#050505] overflow-hidden border-b border-white/5">
        ...
        {item.status === 'success' && (
          <div className="absolute inset-0 z-50 bg-green-900/20 backdrop-blur-sm flex items-center justify-center border border-green-500/30">
        ...
        <div className="absolute top-3 right-3 z-20 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className={`p-2 rounded-lg backdrop-blur-md border ${isSelected ? 'bg-cyan-500 border-cyan-400 text-white' : 'bg-black/60 border-white/20 text-gray-300'}`}
          >
            <CheckSquare size={14} />
          </button>
          <button
            onClick={() => onRemove(item.id)}
            className="p-2 rounded-lg bg-red-500/20 text-red-400 border border-red-500/30 backdrop-blur-md"
          >
            <Trash2 size={14} />
          </button>
        </div>
        <div className="absolute bottom-3 left-3 z-20">
          <span className="text-[10px] font-mono font-bold bg-black/80 text-cyan-400 px-2 py-1 rounded border border-cyan-500/20 backdrop-blur-md shadow-lg">
            {smartCodePreview}
          </span>
        </div>
        <CssImageEditor
      ...
      <div className="p-4 space-y-3 bg-black flex-1 flex flex-col">
        ...
        <div className="bg-white/5 rounded-xl p-2 border border-white/10 flex justify-between items-center">
          <div className="flex flex-col w-1/2 border-r border-white/10 pr-2">
            <label
              className={`text-[8px] font-black uppercase tracking-wider ${isByWeight ? 'text-purple-400' : 'text-cyan-500'}`}
            >
            ...
            <input
              ...
              className="bg-transparent text-sm font-mono font-bold text-white outline-none"
              placeholder="0.00"
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="relative">
            <span className="absolute top-2 left-2 text-[8px] text-gray-500 font-bold">
              CM
            </span>
            <input
              value={item.data.cm}
              onChange={(e) => onUpdate(item.id, 'cm', e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-lg py-1.5 pl-6 text-[10px] text-white outline-none focus:border-cyan-500 uppercase"
              placeholder="45"
            />
          </div>
          <div className="relative">
        ...
        <div className="pt-2 mt-auto">
          <div className="flex justify-between items-center mb-1">
            <label className="text-[8px] font-black text-gray-600 uppercase flex items-center gap-1">
              <Package size={10} /> Grades
            </label>
            <div className="flex gap-1">
              <button
                onClick={() => onAddVariante(item.id, 'cm')}
                className="text-[8px] font-bold bg-white/10 text-white px-1.5 py-0.5 rounded border border-white/10 hover:bg-[#d19900] hover:text-black hover:border-[#d19900] transition-colors"
              >
                +CM
              </button>
              <button
                onClick={() => onAddVariante(item.id, 'aro')}
                className="text-[8px] font-bold bg-white/10 text-white px-1.5 py-0.5 rounded border border-white/10 hover:bg-[#d19900] hover:text-black hover:border-[#d19900] transition-colors"
              >
                +ARO
              </button>
          ...
          <div className="space-y-1 max-h-24 overflow-y-auto scrollbar-hide">
            {item.data.variantes.map((v: ProdutoVariante, idx: number) => (
              <div
                key={idx}
                className="flex items-center gap-2 bg-white/5 p-1 rounded border border-white/5 group/v"
              >
                <input
                  value={v.medida}
                  onChange={(e) =>
                    onUpdateVariante(item.id, idx, 'medida', e.target.value)
                  }
                  className="w-10 bg-transparent text-[9px] text-white outline-none font-bold"
                  placeholder="Tam"
                />
                <div className="flex items-center gap-0.5 border-l border-white/10 pl-1.5">
                  ...
                  <input
                    ...
                    className="w-10 bg-transparent text-[9px] text-white outline-none"
                  />
                </div>
                <button
                  onClick={() =>
                  ...
                  className={`ml-auto text-[7px] font-black px-1 py-0.5 rounded ${v.sob_consulta ? 'text-red-400' : 'text-green-400'}`}
                >
                  {v.sob_consulta ? 'CONS' : 'OK'}
                </button>
                <button
                  onClick={() => onRemoveVariante(item.id, idx)}
                  className="text-gray-600 hover:text-red-400 opacity-0 group-hover/v:opacity-100 transition-opacity"
                >
                  <Trash2 size={10} />
                </button>
              </div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-2">
          ...
          <select
            value={item.data.supplierId}
            onChange={(e) => onUpdate(item.id, 'supplierId', e.target.value)}
            className="bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-[9px] text-gray-400 outline-none"
          >
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none border border-white/5 opacity-30">
        ...
        <div className="absolute left-1/2 h-full w-px bg-white/10"></div>
      </div>
    </div>
  );
}
