...
export function CampanhasPage() {
  ...
  const handleApply = async () => {
    if (
      !confirm(
        `TEM CERTEZA? Isso vai aplicar ${discount}% de desconto em TODO o site.`,
      )
    )
      return;
  ...
  const handleRevert = async () => {
    if (
      !confirm(
        "Isso vai remover todas as promoções e voltar aos preços originais.",
      )
    )
      return;
                ...
                <div className="mb-6">
                  ...
                  <div className="w-full h-4 bg-gray-100 rounded-full overflow-hidden flex relative">
                    ...
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${(simulation.projectedRevenue / simulation.currentRevenue) * 100}%`,
                      }}
                  ...
                  <p className="text-xs text-right text-red-500 mt-1 font-medium">
                    Queda de{" "}
                    {formatCurrency(
                      simulation.currentRevenue - simulation.projectedRevenue,
                    )}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="w-full h-4 bg-gray-100 rounded-full overflow-hidden flex relative">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${(simulation.projectedProfit / simulation.currentProfit) * 100}%`,
                      }}
                  ...
                  {simulation.projectedProfit <= 0 && (
                    <div className="mt-2 flex items-center gap-2 text-red-600 bg-red-50 p-2 rounded-lg text-xs font-bold">
                      <AlertTriangle size={14} /> ALERTA: Esta promoção pode
