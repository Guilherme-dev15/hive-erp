  ...
  const adicionar = useCallback((produto: ProdutoCatalogo) => {
    ...
    setCarrinho((prev) => {
      ...
      const finalPrice =
        ...
        produto.promotionalPrice < produto.salePrice
          ? produto.promotionalPrice
          : produto.salePrice;
...
const useProductFilter = (produtos: ProdutoCatalogo[]) => {
  ...
  const filtered = useMemo(() => {
    ...
    const getPrice = (p: ProdutoCatalogo) =>
      p.isOnSale && p.promotionalPrice ? p.promotionalPrice : p.salePrice;

    if (sortOrder === 'priceAsc')
      lista.sort((a, b) => getPrice(a) - getPrice(b));
    if (sortOrder === 'priceDesc')
      lista.sort((a, b) => getPrice(b) - getPrice(a));
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-800 mb-2">
          Ops! Loja indisponível
        </h2>
        <p className="text-gray-500 max-w-xs">{error}</p>
        <p className="text-xs text-gray-400 mt-4">
          Tente usar ?loja=nome-da-loja
        </p>
      </div>
    );
        ...
        <div className="max-w-7xl mx-auto">
          <div className="px-4 mb-3 flex gap-2">
            ...
            <button
              ...
              className="p-3 rounded-2xl shadow-sm bg-white text-gray-500"
            >
