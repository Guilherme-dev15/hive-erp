    ...
    async function carregarLoja() {
      try {
        ...
        const [prodRes, confRes, catRes] = await Promise.all([
          ...
          getConfigPublica().catch(() => ({ whatsappNumber: null, storeName: '', primaryColor: '', secondaryColor: '' } as ConfigPublica)),
...
function ModalCarrinho({ isOpen, onClose, itens, setCarrinho, whatsappNumber, config }: ModalCarrinhoProps) {
  ...
  const atualizarQuantidade = (id: string, delta: number) => {
    setCarrinho(prev => {
      ...
      const stock = item.produto.quantity || 0;
                ...
                {itens.map((item: any) => (
                  <div key={item.produto.id} className="flex justify-between items-center border-b pb-2">
                     ...
                     <div className="flex items-center border rounded bg-white">
                        <button onClick={() => atualizarQuantidade(item.produto.id, -1)} className="p-1 px-2 hover:bg-gray-100"><Minus size={14}/></button>
                        <span className="px-2 text-sm font-bold">{item.quantidade}</span>
                        <button onClick={() => atualizarQuantidade(item.produto.id, 1)} className="p-1 px-2 hover:bg-gray-100"><Plus size={14}/></button>
                     </div>
                  </div>
                ))}
             </div>
