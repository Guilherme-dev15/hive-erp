...
export function PedidosPage() {
    ...
    const atualizarEstadoPedido = useCallback((id: string, novoStatus: OrderStatus) => {
    ...
    const carregarDados = useCallback(async (silencioso = false) => {
        try {
            ...
            const sortedPedidos = pedidosData
                .filter((p: any) => p && p.id)
                .sort((a: Order, b: Order) => getDateSeconds(b.createdAt) - getDateSeconds(a.createdAt));
    ...
    if (loading) return <div className="h-[80vh] flex flex-col items-center justify-center"><Loader2 className="animate-spin text-indigo-600 mb-2" size={40} /><p className="text-gray-400 font-medium text-sm">Carregando pedidos...</p></div>;
            ...
            <div className="space-y-8 pb-20">
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-4">
                    <div className="flex overflow-x-auto pb-2 no-scrollbar gap-2 border-b border-gray-100">
                        <button onClick={() => setStatusFilter('Todos')} className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all ${statusFilter === 'Todos' ? 'bg-gray-900 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>Todos</button>
                        {statusOrdem.map(status => (
                            <button key={status} onClick={() => setStatusFilter(status)} className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all ${statusFilter === status ? 'bg-gray-100 text-gray-900 ring-1 ring-gray-200' : 'text-gray-500 hover:bg-gray-50'}`}>{status}</button>
                        ))}
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="relative flex-grow">
                            ...
                            <input type="text" placeholder="Buscar por ID, Nome ou Telefone..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm font-medium focus:bg-white focus:ring-2 focus:ring-indigo-600 outline-none transition-all" />
                        </div>
                        <div className="relative min-w-[200px]">
                            <Calendar className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                            <select value={dateFilter} onChange={(e) => setDateFilter(e.target.value as any)} className="w-full pl-10 pr-8 py-2.5 bg-white border border-gray-200 rounded-lg text-sm font-medium focus:ring-2 focus:ring-indigo-600 outline-none appearance-none cursor-pointer">
                ...
                {viewMode === 'list' ? (
                            ...
                            <tbody className="divide-y divide-gray-100">
                                <AnimatePresence>
                                    {pedidosFiltrados.map(pedido => {
                                        ...
                                        return (
                                            <motion.tr key={pedido.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="hover:bg-gray-50/80 transition-colors group">
                                                ...
                                                <td className="px-6 py-4">
                                                    {updatingId === pedido.id ? (
                                                        ...
                                                        <select value={pedido.status} onChange={(e) => handleStatusChange(pedido.id, e.target.value as OrderStatus)} className={`appearance-none px-3 py-1.5 rounded-full text-xs font-bold border ${statusStyle.bg} ${statusStyle.color} ${statusStyle.border} outline-none cursor-pointer`}>
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex justify-center gap-2">
                                                        <button onClick={() => prepararEImprimirCertificado(pedido)} className="p-2 text-emerald-600 bg-emerald-50 rounded-lg hover:bg-emerald-100 transition-colors"><ScrollText size={16} /></button>
                                                        <button onClick={() => { setPedidoSelecionado(pedido); setModalOpen(true); }} className="p-2 text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors"><Search size={16} /></button>
                                                        <button onClick={() => handleDelete(pedido.id)} className="p-2 text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors"><Trash2 size={16} /></button>
                                                    </div>
                                                </td>
                                            </motion.tr>
                                        );
                    ...
                    <div className="flex overflow-x-auto pb-6 gap-4 items-start custom-scrollbar">
                        {statusOrdem.map(status => {
                            const pedidosDoStatus = pedidosFiltrados.filter(p => p.status === status || (status === 'Aguardando Pagamento' && !p.status));
                            const estilo = statusConfig[status];
                            return (
                                <div key={status} className="min-w-[280px] w-[280px] bg-gray-50 rounded-xl border border-gray-200 flex flex-col max-h-[calc(100vh-250px)]">
                                    <div className={`p-3 border-b border-gray-200 bg-white rounded-t-xl flex justify-between items-center ${estilo.color}`}>
                                    ...
                                    <div className="p-2 space-y-2 overflow-y-auto flex-1">
                                        {pedidosDoStatus.map(pedido => (
                                            <motion.div key={pedido.id} layoutId={pedido.id} onClick={() => { setPedidoSelecionado(pedido); setModalOpen(true); }} className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm hover:shadow-md cursor-pointer transition-all">
            ...
            <DetalhePedidoModal
                ...
                onUpdate={(novoStatus: string) => {
                    if (pedidoSelecionado) {
                        atualizarEstadoPedido(pedidoSelecionado.id, novoStatus as OrderStatus);
                    }
                }}
            />

            <CertificadoImpressao ref={certificadoRef} pedido={pedidoParaCertificado} config={config} />
        </>
    );
}