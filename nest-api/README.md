...
5. O fluxo canônico cobre usuários, categorias, fornecedores, produtos, pedidos e transações. Variantes, cupons, configurações e logs de estoque exigem uma etapa específica antes de serem migrados.
6. Só execute `npm run migrate:legacy` em ambiente de homologação, após backup e aprovação explícita. Nunca use o comando contra produção sem um plano de rollback.
